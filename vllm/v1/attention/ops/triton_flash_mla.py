...
def flash_mla_decode_stage_1_kernel(
    ...
    # load Q
    qn = tl.load(Q_nope_ptr + pid_b * stride_qnb + pid_h * stride_qnh + offs_d_latent * stride_qnd).to(tl.float16)
    qp = tl.load(Q_pe_ptr + pid_b * stride_qpb + pid_h * stride_qph + offs_d_rope * stride_qpd).to(tl.float16)
    ...
    for curr_n in range(start_n, end_n, BLOCK_N):
        ...
        # read separately to avoid not a power of 2 error
        ...
        # accumulate in fp32
        acc = acc * alpha + tl.sum(kn.to(tl.float32) * p[:, None], 0)
...
def flash_mla_decode_fused_kernel(
    ...
    for curr_n in range(0, seq_len, BLOCK_N):
        offs_n = curr_n + tl.arange(0, BLOCK_N)
    ...
    tl.store(out_ptr, acc.to(Output_ptr.dtype.element_ty))